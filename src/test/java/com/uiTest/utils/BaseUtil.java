...
import com.uiTest.constants.global.GlobalConstants;
    ...
    public WebDriver createDriver(String url, String browser) {
        ...
        // Initialize driver object to CHROME BROWSER
            ...
            if (Platform.getCurrent().toString().matches(".*?(mac|MAC).*?")) {
                ...
                chromeOptions.addArguments("--disable-blink-features=AutomationControlled"); // 让selenium 不被网站识别
            }
            ...
            log.info("Set global implicit wait time: " + GlobalConstants.TIMEOUT + " seconds");
        }
...
//        driver.manage().window().fullscreen();
    ...
    // WAIT FOR MAX TIME GlobalConstants.TIMEOUT SECS TILL THE ELEMENT IS CLICKABLE - DISPLAYED AND ENABLED
    protected WebElement wait_until_ElementIs_Clickable(WebDriver driver, By locator) {
        wait = new WebDriverWait(driver, GlobalConstants.TIMEOUT);
    ...
    // DROPDOWN SELECTOR
    public void selectFromDropdownByValue(WebElement selector, String value) {
        Select select = new Select(selector);
        select.selectByValue(value);
    }

    public void selectFromDropdownByIndex(WebElement selector, int index) {
        Select select = new Select(selector);
    ...
    // WAIT FOR MAX TIME GlobalConstants.TIMEOUT SECS TILL THE ELEMENT IS VISIBLE
